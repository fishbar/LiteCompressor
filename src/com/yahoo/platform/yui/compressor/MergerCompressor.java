...
public class MergerCompressor {
    ...
    static {
        ...
        literals.put(new Integer(Token.INCLUDE),"$include");
        ...
        while (offset < length) {
            ...
            switch (tt) {
                ...
                case Token.STRING:
                    ...
                    tokens.add(new JavaScriptToken(tt, escape(sb.toString())));
                    break;
    ...
    */
    ...
    private static boolean disableOptimizations;
    private static int linebreak;
    private static String codeBase;
    private static String charset;
    private static Hashtable<String,MergerCompressor> cached_mod = new Hashtable<String,MergerCompressor>();
    private static Hashtable<String,Boolean> included_mod;
    ...
    public MergerCompressor(boolean rootfile){
        if(rootfile){
        	included_mod = new Hashtable<String,Boolean>();
        }
    }
    public static void config(String _codeBase,String _charset,int _linebreak, boolean _munge, boolean _verbose,
            boolean _preserveAllSemiColons, boolean _disableOptimizations){
    	linebreak = _linebreak;
    	munge = _munge;
    	verbose = _verbose;
    	preserveAllSemiColons = _preserveAllSemiColons;
    	disableOptimizations = _disableOptimizations;
    	codeBase = _codeBase;
    	charset = _charset;
    }
    public void loadScript(Reader in,ErrorReporter reporter)throws IOException, EvaluatorException{
    	this.logger = reporter;
    	tokens = parse(in, logger);
    }
    public void loadScript(Reader in)throws IOException, EvaluatorException{
    	loadScript(in,new LiteReporter());    	
    }
    ...
    public StringBuffer printSymbolTree(int linebreakpos, boolean preserveAllSemiColons)
        ...
        while (offset < length) {
            ...
            switch (token.getType()) {
            	case Token.COMMA:
            		JavaScriptToken next = getToken(0);
            		if(next.getType() == Token.RC){
            			continue;
            		}else{
                ...
                case Token.KEEPCOMMENT:
                    ...
                    break;
                case Token.INCLUDE:
                	token = consumeToken();	//get next token 
                	// TODO fetch compile File
                	// fileName = token.getValue();
                	//result.append("fetch(");
                	MergerCompressor cc;
                	String mod_path = token.getValue();
                	String mod_cnt;
                	String sub_charset = charset;
                	Reader mod_in;
                	MergerCompressor mod_obj = cached_mod.get(mod_path);
                	token = consumeToken();
                	if(token.getType() == Token.SEMI){
                		// do nothing
            		}else if(token.getType() == Token.NAME){ // file encode
                		sub_charset = token.getValue();
                		consumeToken();
                	}
